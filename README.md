...
#### Conversion to TensorRT format (for the installation instuctions of TensorRT and its requirements follow the official domumentation and [installation guide](https://docs.nvidia.com/deeplearning/tensorrt/install-guide/index.html) from Nvidia)
1. Convert PyTorch model to ONNX format: run script in terminal `python scripts/convert_to_onnx.py --checkpoint-path <CHECKPOINT>`. It produces `human-pose-estimation.onnx`. Alternatively execute the python demo script as shown below and use the `--export-onnx` flag to get the output file `checkpoints/pose_estimation_with_mobilenet.onnx` like that `python demo.py --checkpoint-path checkpoints/checkpoint_iter_370000.pth --video 0 --export-onnx`.
2. Convert the exported ONNX model to TensorRT engine format. In order to do so you can execute the following command in terminal after you move to the *checkpoints* folder: `trtexec --onnx=pose_estimation_with_mobilenet.onnx --saveEngine=pose_estimation_with_mobilenet_fp16.engine --workspace=5120 --precisionConstraints=prefer --fp16`. You can either use `--fp16` or not to produce a TensorRT engine with half precision support or not. The `workspace` param dictates the GPU memory given to TensorRT compiler while searching to produce the final optimized engine. For more options for the conversion execute `trtexec --help` or check the official TensorRT [documentation](https://docs.nvidia.com/deeplearning/tensorrt/developer-guide/index.html).
3. In order to try the python live demo using the exported TensorRT engine execute the python script as shown below using the `--trt-inferece` flag like that `python demo.py --checkpoint-path checkpoints/checkpoint_iter_370000.pth --video 0 --trt-inferece`.
